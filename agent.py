...
"""
        ...
        if parse_result['parameters']:
            ...
            for key, value in parse_result['parameters'].items():
                if isinstance(value, list):
                    print(f"{Fore.YELLOW}    - {key}: {', '.join(str(v) for v in value)}")
                else:
    ...
    def display_result_data(self, data: Dict[str, Any]):
        ...
        # 特殊处理批量操作结果
            ...
            success = data.get('archived', data.get('deleted', data.get('forwarded', data.get('success_count', data.get('marked', 0)))))
            ...
            # 显示详细结果（只显示失败的）
            failed_items = [r for r in data['results'] if r.get('status') not in ['success', 'archived', 'deleted', 'forwarded', 'marked_read', 'marked_unread']]
            if failed_items:
                print(f"\n{Fore.RED}失败项目:")
                for item in failed_items:
                    if 'subject' in item:
                        print(f"  {Fore.YELLOW}• {item.get('subject', '未知')}: {item.get('status', '未知错误')}")
                    elif 'recipient' in item:
                        print(f"  {Fore.YELLOW}• {item.get('recipient', '未知')}: {item.get('status', '未知错误')}")
        ...
        # 特殊处理优先级分析
            ...
            print(f"  {Fore.CYAN}是否重要: {'是' if analysis.get('is_important') else '否'}")
        ...
        # 通用数据显示
            ...
            for key, value in data.items():
                if key not in ['emails', 'summaries', 'results', 'priority_analysis', 'reply_content', 'summary']:
                    if isinstance(value, (dict, list)):
                        print(f"  {Fore.CYAN}{key}: {json.dumps(value, ensure_ascii=False, indent=2)}")
                    else:
