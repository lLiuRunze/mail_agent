...
class EmailClient:
    ...
    def get_email(self, email_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            # 选择收件箱
            self.imap_connection.select(Config.DEFAULT_FOLDER) # type: ignore

            # 获取邮件
            status, data = self.imap_connection.fetch(email_id, "(RFC822)") # type: ignore
    ...
    def get_recent_emails(
        ...
        try:
            ...
            self.imap_connection.select(folder) # type: ignore
